...
L.GeoTime = L.Class.extend({
    ...
    _geoToLayer: function() {
        for (var i = 0, len = this._geos.length; i < len; i++) {
            ...
            this._ticks.push(props.index.length);
            this._tags.push(0);
            this._initTime(props);
            switch (geometry.type) {
                case 'Point':
                    var marker = this._createMaker(geometry.coordinates, props.index[0].val, props.name);
                    this._features.push(marker)
                    break;
                case 'MultiPoint':
                    var makerGroup = [];
                    for (j = 0, len = geometry.coordinates.length; j < len; j++) {
                        var maker = this._createMaker(geometry.coordinates[j], props.index[0].val, props.name)
                ...
                case 'MultiLineString':
                    var lines = L.polyline(this._coordsToLatLngs(geometry.coordinates, geometry.type === 'LineString' ? 0 : 1, this._coordsToLatLng), {
                        color: this._getColor(props.index[0].val),
                        type: "Line",
                        id: props.name
                    });
                ...
                case 'MultiPolygon':
                    var polygons = L.polygon(this._coordsToLatLngs(geometry.coordinates, geometry.type === 'Polygon' ? 1 : 2, this._coordsToLatLng), {
                        fillColor: this._getColor(props.index[0].val),
                        weight: 2,
                        opacity: 1,
                        color: 'white',
                        dashArray: '3',
    ...
    _initTime: function(props) {
        var time = this._geos[0].properties.index;
        this._earliestTime = new Date(time[0].time).getTime();

        this._latestTime = time[time.length - 1][0];
        if (new Date(time[0].time).getTime() < this._earliestTime) {
            this._earliestTime = new Date(time[0].time).getTime();
        }
        if (new Date(props.index[props.index.length - 1].time).getTime() > this._latestTime) {
            this._latestTime = new Date(props.index[props.index.length - 1].time).getTime();
        }
    },
    _isPlaying: function() {
        return this._intervalID ? true : false;
    ...
    _animation: function(self) {
        ...
        for (var i = 0, len = self._features.length; i < len; i++) {
            if (new Date(self._geos[i].properties.index[self._geos[i].properties.index.length - 1].time).getTime() < self._earliestTime + self._tick * self._duration) {
                continue;
            }

            // console.log(self._earliestTime + self._tick * self._duration);
            if (self._features[i].options.type == "Line" && new Date(self._geos[i].properties.index[self._tags[i]].time).getTime() <= self._earliestTime + self._tick * self._duration) {
                self._features[i].setStyle({
                    color: self._getColor(self._geos[i].properties.index[self._tags[i]].val)
                });
                self._tags[i] = self._tags[i] + 1;
            } else if (new Date(self._geos[i].properties.index[self._tags[i]].time).getTime() <= self._earliestTime + self._tick * self._duration) {
                self._features[i].setStyle({
                    fillColor: self._getColor(self._geos[i].properties.index[self._tags[i]].val)
                });
                self._tags[i] = self._tags[i] + 1;
...
export const geoTime = function(geoJSON, options) {
    return new L.GeoTime(geoJSON, options);
}
