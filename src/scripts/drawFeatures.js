...
export const selectDevice = (data) => {
	...
	let ydDeviceLayer = new L.FeatureGroup();
	map.addLayer(ydDeviceLayer);

	var ydDevicedrawControl = new L.Control.Draw({
		edit: {
			featureGroup: ydDeviceLayer,
		},
		draw: {
			polyline: false,
			polygon: {
	...
	map.addControl(ydDevicedrawControl);
	var latlngs = [],
		coords = null;
	...
	map.on('draw:created', function(e) {
		...
		//第一个点和最后一个点要保持一致，要不turf解析不了。
		latlngs.push(latlngs[0])
		selectDevLayer = turf.polygon([latlngs]);
		var fc_selectDevLayer = {
			"type": "FeatureCollection",
			"features": [selectDevLayer]
		};
		var pointsWithin_Region = turf.within(jsonPoints, fc_selectDevLayer);
		var deviceWithin = [];
		pointsWithin_Region.features.map((p1) => {
			deviceWithin.push(p1.properties.id);
		});
		console.log('send', deviceWithin);
		lmsg.send('querySelectedXh', {
			ids: deviceWithin
		});
		map.removeEventListener('draw:created');
		map.removeControl(ydDevicedrawControl);
	});
